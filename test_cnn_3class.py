...
def get_train_multi(type_name='train'):
	...
	with h5py.File('three_class/'+type_name+'_three_class.h5', 'r') as hf:
		X = hf['data'][:]
	hf.close()
	with h5py.File('three_class/labels_'+type_name+'_three_class.h5', 'r') as hf:
		y = hf['data'][:]
	hf.close()
	return X, y
...
def grab_model(type_name, X_val_new):
	if type_name=='branch':
		...
		model = load_model("model_cnn_3class0_gauss_512_3conv.h5")
...
def get_cm_err():
	...
	meds = numpy.median(mats_glob, axis=0)
...
def save_cm(y_val_new, predictions, save_name='nh3', class_names=class_names):
...
def get_confusion_matrix(type_name='branch'):
	...
	#x2[x2==2] = 1
	x2g = numpy.load('chi_predictions_3class_global_test_bic.npy')
	x2l = numpy.load('chi_predictions_3class_local_test_bic.npy')
	...
	class_names = ['one', 'noise', 'two']
	X_val_new2, y_val_new2 = get_train_multi(type_name='test_gauss')
	X_val_new, y_val_new = get_train_multi_nh3(type_name='test')
	...
	model2, X_val_new2 = grab_model(type_name='branch', X_val_new=X_val_new2)
	...
	print 'CNN NH3:'
	save_cm(y_val_new, predictions, save_name='nh3', class_names=class_names)

	print 'Chi-global NH3:'
	save_cm(y_val_new, x2g_nh3, save_name='chi_nh3_global', class_names=class_names)

	print 'Chi-local NH3:'
	save_cm(y_val_new, x2l_nh3, save_name='chi_nh3_local', class_names=class_names)

	print 'CNN Gauss:'
	save_cm(y_val_new2, predictions2, save_name='branch', class_names=class_names)

	print 'Chi-global Gauss:'
	save_cm(y_val_new2, x2g, save_name='chi_gauss_global', class_names=class_names)

	print 'Chi-local Gauss:'
	save_cm(y_val_new2, x2l, save_name='chi_gauss_local', class_names=class_names)
