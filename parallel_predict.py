...
def para_predict(modname):
    ...
    model = load_model(modname[0])
    prediction = model.predict(modname[1])
    return prediction
...
def ensemble_predictions(members, testX, nproc):
    ...
    members = zip(members, [testX for i in range(6)])
    results = pool.map(para_predict, members)

    # make predictions
    #yhats = [model.predict(testX) for model in members]
    yhats = numpy.array(results)
